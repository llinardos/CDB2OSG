...
void GeometryTechnique::init(int dirtyMask, bool assumeMultiThreaded) {
    OSG_INFO << "Doing CDB::GeometryTechnique::init()" << std::endl;
    ...
    osg::ref_ptr<osgTerrain::TerrainTile> tile = _terrainTile;
    ...
    if ((dirtyMask & osgTerrain::TerrainTile::IMAGERY_DIRTY) == 0) {
...
osgTerrain::Locator *GeometryTechnique::computeMasterLocator() {
...
osg::Vec3d GeometryTechnique::computeCenterModel(BufferData &buffer, osgTerrain::Locator *masterLocator) {
...
public:
    ...
    typedef std::pair<osg::ref_ptr<osg::Vec2Array>, osgTerrain::Locator *> TexCoordLocatorPair;
    typedef std::map<osgTerrain::Layer *, TexCoordLocatorPair> LayerToTexCoordMap;

    VertexNormalGenerator(osgTerrain::Locator *masterLocator, const osg::Vec3d &centerModel, int numRows, int numColmns,
                          float scaleHeight, bool createSkirt);
    ...
    inline int vertex_index(int c, int r) const {
        int i = _indices[(r + 1) * (_numColumns + 2) + c + 1];
        return i - 1;
    }
...
VertexNormalGenerator::VertexNormalGenerator(osgTerrain::Locator *masterLocator, const osg::Vec3d &centerModel,
                                             int numRows, int numColumns, float scaleHeight, bool createSkirt) :
                ...
                for (VertexNormalGenerator::LayerToTexCoordMap::iterator itr = layerToTexCoordMap.begin();
                    ...
                    if (imageLayer != NULL) {
                        ...
                        if (colorLocator != _masterLocator) {
                            osg::Vec3d color_ndc;
                            osgTerrain::Locator::convertLocalCoordBetween(*_masterLocator, ndc, *colorLocator,
                                                                          color_ndc);
                        ...
                        if (contourLayer) {
                            ...
                            if (transferFunction) {
                                ...
                                if (difference != 0.0f) {
                                    ...
                                    if (colorLocator != _masterLocator) {
                                        osgTerrain::Locator::convertLocalCoordBetween(*_masterLocator, ndc,
                                                                                      *colorLocator, color_ndc);
                                    }
                                    else {
                                        color_ndc = ndc;
                                    }
                                    ...
                                    (*texcoords).push_back(
                                            osg::Vec2((color_ndc[2] - transferFunction->getMinimum()) / difference,
                                                      0.0f));
                                    texCoordSet = true;
                                }
                            }
...
void GeometryTechnique::generateGeometry(BufferData &buffer, osgTerrain::Locator *masterLocator,
    ...
    if (terrain && terrain->getEqualizeBoundaries()) {
        osgTerrain::TileID tileId = _terrainTile->getTileID();

        osg::ref_ptr<osgTerrain::TerrainTile> left_tile = terrain->getTile(
                osgTerrain::TileID(tileId.level, tileId.x - 1, tileId.y));
        osg::ref_ptr<osgTerrain::TerrainTile> right_tile = terrain->getTile(
                osgTerrain::TileID(tileId.level, tileId.x + 1, tileId.y));
        osg::ref_ptr<osgTerrain::TerrainTile> top_tile = terrain->getTile(
                osgTerrain::TileID(tileId.level, tileId.x, tileId.y + 1));
        osg::ref_ptr<osgTerrain::TerrainTile> bottom_tile = terrain->getTile(
                osgTerrain::TileID(tileId.level, tileId.x, tileId.y - 1));

#if 0
        osg::ref_ptr<osgTerrain::TerrainTile> top_left_tile  = terrain->getTile(DatasetID(lodTile.level, lodTile.x-1, lodTile.y+1));
        osg::ref_ptr<osgTerrain::TerrainTile> top_right_tile = terrain->getTile(DatasetID(lodTile.level, lodTile.x+1, lodTile.y+1));
        osg::ref_ptr<osgTerrain::TerrainTile> bottom_left_tile = terrain->getTile(DatasetID(lodTile.level, lodTile.x-1, lodTile.y-1));
        osg::ref_ptr<osgTerrain::TerrainTile> bottom_right_tile = terrain->getTile(DatasetID(lodTile.level, lodTile.x+1, lodTile.y-1));
#endif
        ...
        if (left_tile.valid()) {
            if (!(left_tile->getTerrainTechnique()->containsNeighbour(_terrainTile))) {
                int dirtyMask = left_tile->getDirtyMask() | osgTerrain::TerrainTile::LEFT_EDGE_DIRTY;
        ...
        if (right_tile.valid()) {
            if (!(right_tile->getTerrainTechnique()->containsNeighbour(_terrainTile))) {
                int dirtyMask = right_tile->getDirtyMask() | osgTerrain::TerrainTile::RIGHT_EDGE_DIRTY;
        ...
        if (top_tile.valid()) {
            if (!(top_tile->getTerrainTechnique()->containsNeighbour(_terrainTile))) {
                int dirtyMask = top_tile->getDirtyMask() | osgTerrain::TerrainTile::TOP_EDGE_DIRTY;
        ...
        if (bottom_tile.valid()) {
            if (!(bottom_tile->getTerrainTechnique()->containsNeighbour(_terrainTile))) {
                int dirtyMask = bottom_tile->getDirtyMask() | osgTerrain::TerrainTile::BOTTOM_EDGE_DIRTY;
        ...
        if (bottom_left_tile.valid())
        {
            ...
            {
                int dirtyMask = bottom_left_tile->getDirtyMask() | osgTerrain::TerrainTile::BOTTOM_LEFT_CORNER_DIRTY;
        ...
        if (bottom_right_tile.valid())
        {
            ...
            {
                int dirtyMask = bottom_right_tile->getDirtyMask() | osgTerrain::TerrainTile::BOTTOM_RIGHT_CORNER_DIRTY;
        ...
        if (top_right_tile.valid())
        {
            ...
            {
                int dirtyMask = top_right_tile->getDirtyMask() | osgTerrain::TerrainTile::TOP_RIGHT_CORNER_DIRTY;
        ...
        if (top_left_tile.valid())
        {
            ...
            {
                int dirtyMask = top_left_tile->getDirtyMask() | osgTerrain::TerrainTile::TOP_LEFT_CORNER_DIRTY;
    ...
    osg::ref_ptr<osg::DrawElements> elements = smallTile ?
                                               static_cast<osg::DrawElements *>(new osg::DrawElementsUShort(
                                                       GL_TRIANGLES)) :
                                               static_cast<osg::DrawElements *>(new osg::DrawElementsUInt(
                                                       GL_TRIANGLES));
    ...
    for (unsigned int layerNum = 0; layerNum < _terrainTile->getNumColorLayers(); ++layerNum) {
        ...
        if (imageLayer) {
            ...
            if (!texture2D) {
                ...
                bool mipMapping = !(texture2D->getFilter(osg::Texture::MIN_FILTER) == osg::Texture::LINEAR ||
                                    texture2D->getFilter(osg::Texture::MIN_FILTER) == osg::Texture::NEAREST);
                ...
                if (mipMapping && (s_NotPowerOfTwo || t_NotPowerOfTwo)) {
                    OSG_INFO << "Disabling mipmapping for non power of two tile size(" << image->s() << ", " <<
                             image->t() << ")" << std::endl;
...
void GeometryTechnique::applyTransparency(BufferData &buffer) {
    osgTerrain::TerrainTile::BlendingPolicy blendingPolicy = _terrainTile->getBlendingPolicy();
    if (blendingPolicy == osgTerrain::TerrainTile::INHERIT && _terrainTile->getTerrain()) {
    ...
    if (blendingPolicy == osgTerrain::TerrainTile::INHERIT) {
        OSG_INFO <<
                 "GeometryTechnique::applyTransparency() policy is INHERIT, defaulting to ENABLE_BLENDING_WHEN_ALPHA_PRESENT" <<
                 std::endl;
        blendingPolicy = osgTerrain::TerrainTile::ENABLE_BLENDING_WHEN_ALPHA_PRESENT;
    }

    if (blendingPolicy == osgTerrain::TerrainTile::DO_NOT_SET_BLENDING) {
        OSG_INFO << "blendingPolicy == osgTerrain::TerrainTile::DO_NOT_SET_BLENDING" << std::endl;
        return;
    }

    bool enableBlending = false;

    if (blendingPolicy == osgTerrain::TerrainTile::ENABLE_BLENDING) {
        OSG_INFO << "blendingPolicy == osgTerrain::TerrainTile::ENABLE_BLENDING" << std::endl;
        enableBlending = true;
    }
    else if (blendingPolicy == osgTerrain::TerrainTile::ENABLE_BLENDING_WHEN_ALPHA_PRESENT) {
        OSG_INFO << "blendingPolicy == osgTerrain::TerrainTile::ENABLE_BLENDING_WHEN_ALPHA_PRESENT" << std::endl;
